...
const float VOLTAGE_DIVIDER_FACTOR = 2.67;
...
#define HAS_OLED 0
#define PIN_PWM_L1 D7
#define PIN_PWM_L2 D0
#define PIN_PWM_R1 D5
#define PIN_PWM_R2 D6
#define PIN_SPEED_LF TX
#define PIN_SPEED_RF RX
#define PIN_VIN A0
...
//------------------------------------------------------//
  ...
  if ((millis() - sonar_time) >= sonar_interval) {
  ...
  void process_ctrl_msg() {
    ...
    ESP_LOGD(TAG, "Control: %d, %d", ctrl_left, ctrl_right);
    updateMotors();
  }
...
void process_light_msg() {
  ...
  void process_heartbeat_msg() {
    ...
    ESP_LOGD(TAG, "Heartbeat Interval: %d", heartbeat_interval);
  }
...
void update_distance_estimate() {
  if (id(ultrasonic_sensor).has_state()) {
    distance_estimate = id(ultrasonic_sensor).get_state() * 100;
  }
}
