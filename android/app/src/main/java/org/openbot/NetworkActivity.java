...
public class NetworkActivity extends CameraActivity implements OnImageAvailableListener {
  ...
  protected void processImage() {
    ...
    final boolean SAVE_PREVIEW_BITMAP = logMode.equals(LogMode.ALL_IMGS) || logMode.equals(LogMode.PREVIEW_IMG);
    final boolean SAVE_CROP_BITMAP = logMode.equals(LogMode.ALL_IMGS) || logMode.equals(LogMode.CROP_IMG);

    rgbFrameBitmap.setPixels(getRgbBytes(), 0, previewWidth, 0, 0, previewWidth, previewHeight);
    if (getLoggingEnabled() && SAVE_PREVIEW_BITMAP) {
        runInBackground(() -> ImageUtils.saveBitmap(rgbFrameBitmap, logFolder + File.separator + "images", currFrameNum + "_preview.jpeg"));
        if (!SAVE_CROP_BITMAP)
    ...
    if (computingNetwork && !SAVE_CROP_BITMAP){ return;}

    final Canvas canvas = new Canvas(croppedBitmap);
    canvas.drawBitmap(rgbFrameBitmap, frameToCropTransform, null);
    // For examining the actual TF input.
    if (getLoggingEnabled() && SAVE_CROP_BITMAP){
        runInBackground(() -> ImageUtils.saveBitmap(croppedBitmap, logFolder + File.separator + "images", currFrameNum + "_crop.jpeg"));
        sendFrameNumberToSensorService(currFrameNum);
    }
...
//                                + results.get(0).getLocation().width() );
  ...
  private boolean getNoiseEnabled() { return noiseEnabled; }
  private void toggleNoise() { noiseEnabled = !noiseEnabled; }
      ...
      if (autoPilot != null) {
        ...
        try {
          if (model == Model.DETECTOR_V1_1_0_Q || model == Model.DETECTOR_V3_S_Q) {
            LOGGER.d("Creating detector (model=%s, device=%s, numThreads=%d)", model, device, numThreads);
            ...
            LOGGER.d("Creating autopilot (model=%s, device=%s, numThreads=%d)", model, device, numThreads);
            autoPilot = Autopilot.create(this, model, device, numThreads);
            croppedBitmap = Bitmap.createBitmap(autoPilot.getImageSizeX(), autoPilot.getImageSizeY(), Config.ARGB_8888);
            frameToCropTransform =
                    ImageUtils.getTransformationMatrix(
                            previewWidth, previewHeight,
                            croppedBitmap.getWidth(), croppedBitmap.getHeight(),
    ...
    if ((event.getSource() & InputDevice.SOURCE_GAMEPAD) == InputDevice.SOURCE_GAMEPAD
      ...
      switch (event.getKeyCode()) {
        ...
        case KeyEvent.KEYCODE_BUTTON_R1:
          ...
          return true;
        default:
          //Toast.makeText(this,"Key " + event.getKeyCode() + " not recognized", Toast.LENGTH_SHORT).show();
          break;
      }
    }
